...
class HTTPAdapter(ThirdPartyAdapter):
    ...
    def _merge_query_params(self, url: str, params: Optional[Dict]) -> str:
        """
        Fusionne les query params existants dans l'URL avec ceux passés via `params`.
        Les params passés explicitement écrasent ceux déjà présents dans l'URL.
        """
        if not params:
            return url
    ...
    async def _request(
        ...
        final_url = self._merge_query_params(url, params)
        self.logger.debug(f"HTTP {method} → {final_url}")
        ...
        try:
            ...
            raise ConnectionError(
                f"Impossible de se connecter à {final_url}",
                {'original_error': str(e)}
            )

        except httpx.TimeoutException as e:
            raise ConnectionError(
                f"Timeout lors de la connexion à {final_url}",
                {'original_error': str(e)}
            )
    ...
    async def fetch_users(self, **kwargs) -> AdapterResponse:
        ...
        try:
            ...
            self.logger.info(
                f"Récupéré {len(users)} utilisateurs depuis {self.config.name}"
            )
        ...
        for raw in raw_users:
            ...
            users.append(
                UserData(
                    ...
                    is_enabled=raw.get(
                        'is_enabled',
                        raw.get('active', raw.get('enabled', True))
                    ),
                    ...
                    metadata={
                        ...
                        if k not in {
                            'id', 'external_id', 'user_id', 'employee_id',
                            'enrollid', 'fullname', 'full_name', 'name',
                            'display_name', 'is_enabled', 'active', 'enabled',
                            'admin', 'admin_level', 'group', 'department_id',
                            'weekzone', 'start_date', 'end_date',
                            'starttime', 'endtime'
                        }
                    }
                )
